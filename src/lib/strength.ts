...
function levelFromBits(bits: number): StrengthResult {
  if (bits < 40) {
    return { bits, level: 'weak', label: 'Weak', score: Math.max(12, (bits / 40) * 25) }
  }
  if (bits < 60) {
    return { bits, level: 'fair', label: 'Fair', score: 25 + ((bits - 40) / 20) * 25 }
  }
  if (bits < 80) {
    return { bits, level: 'strong', label: 'Strong', score: 50 + ((bits - 60) / 20) * 25 }
  }
